...
public final class Class58_Sub2 extends Class58 {
	...
	private Socket method6107(@OriginalArg(1) Proxy arg0) throws IOException {
		...
		if (arg0.type() == Type.HTTP) {
			...
			local232.connect(new InetSocketAddress(this.aString74, this.anInt6804));
			return local232;
		} else {
			return null;
		}
	...
	private Socket httpProxyConnect(@OriginalArg(0) String arg0, @OriginalArg(1) int arg1, @OriginalArg(2) String arg2) throws IOException {
		...
		if (arg2 == null) {
			local11.write(("CONNECT " + this.aString74 + ":" + this.anInt6804 + " HTTP/1.0\n\n").getBytes(Charset.forName("ISO-8859-1")));
		} else {
			local11.write(("CONNECT " + this.aString74 + ":" + this.anInt6804 + " HTTP/1.0\n" + arg2 + "\n\n").getBytes(Charset.forName("ISO-8859-1")));
		}
		local11.flush();
	...
	public Socket method6104() throws IOException {
		...
		for (@Pc(112) int local112 = 0; local112 < local106.length; local112++) {
			...
			try {
				@Pc(132) Socket local132 = this.method6107(local127);
				if (local132 != null) {
					return local132;
				}
