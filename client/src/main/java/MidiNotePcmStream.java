import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalClass;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

@OriginalClass("client!uka")
public final class MidiNotePcmStream extends PcmStream {

	@OriginalMember(owner = "client!uka", name = "t", descriptor = "Lclient!sia;")
	public final LinkedList aClass341_72 = new LinkedList();

	@OriginalMember(owner = "client!uka", name = "A", descriptor = "Lclient!nn;")
	public final MixerPcmStream aClass2_Sub6_Sub3_2 = new MixerPcmStream();

	@OriginalMember(owner = "client!uka", name = "D", descriptor = "Lclient!bd;")
	private final MidiPcmStream aClass2_Sub6_Sub1_4;

	@OriginalMember(owner = "client!uka", name = "<init>", descriptor = "(Lclient!bd;)V")
	public MidiNotePcmStream(@OriginalArg(0) MidiPcmStream arg0) {
		this.aClass2_Sub6_Sub1_4 = arg0;
	}

	@OriginalMember(owner = "client!uka", name = "a", descriptor = "(I)V")
	@Override
	public void skip(@OriginalArg(0) int arg0) {
		this.aClass2_Sub6_Sub3_2.skip(arg0);
		for (@Pc(15) Node_Sub16 local15 = (Node_Sub16) this.aClass341_72.head(); local15 != null; local15 = (Node_Sub16) this.aClass341_72.next()) {
			if (!this.aClass2_Sub6_Sub1_4.method921(local15)) {
				@Pc(27) int local27 = arg0;
				do {
					if (local27 <= local15.anInt2193) {
						this.method8537(local15, local27);
						local15.anInt2193 -= local27;
						break;
					}
					this.method8537(local15, local15.anInt2193);
					local27 -= local15.anInt2193;
				} while (!this.aClass2_Sub6_Sub1_4.method945(local15, null, 0, local27));
			}
		}
	}

	@OriginalMember(owner = "client!uka", name = "a", descriptor = "(Lclient!dha;II)V")
	private void method8537(@OriginalArg(0) Node_Sub16 arg0, @OriginalArg(2) int arg1) {
		if ((this.aClass2_Sub6_Sub1_4.anIntArray56[arg0.anInt2196] & 0x4) != 0 && arg0.anInt2211 < 0) {
			@Pc(28) int local28 = this.aClass2_Sub6_Sub1_4.anIntArray57[arg0.anInt2196] / AudioChannel.globalSampleRate;
			@Pc(37) int local37 = (local28 + 1048575 - arg0.anInt2192) / local28;
			arg0.anInt2192 = arg0.anInt2192 + local28 * arg1 & 0xFFFFF;
			if (local37 <= arg1) {
				if (this.aClass2_Sub6_Sub1_4.anIntArray49[arg0.anInt2196] == 0) {
					arg0.aClass2_Sub6_Sub2_1 = Static730.method3346(arg0.aClass2_Sub49_Sub1_1, arg0.aClass2_Sub6_Sub2_1.method3340(), arg0.aClass2_Sub6_Sub2_1.method3341(), arg0.aClass2_Sub6_Sub2_1.method3348());
				} else {
					arg0.aClass2_Sub6_Sub2_1 = Static730.method3346(arg0.aClass2_Sub49_Sub1_1, arg0.aClass2_Sub6_Sub2_1.method3340(), 0, arg0.aClass2_Sub6_Sub2_1.method3348());
					this.aClass2_Sub6_Sub1_4.method943(arg0.aClass2_Sub11_1.aShortArray16[arg0.anInt2205] < 0, arg0);
				}
				if (arg0.aClass2_Sub11_1.aShortArray16[arg0.anInt2205] < 0) {
					arg0.aClass2_Sub6_Sub2_1.method3318(-1);
				}
				arg1 = arg0.anInt2192 / local28;
			}
		}
		arg0.aClass2_Sub6_Sub2_1.skip(arg1);
	}

	@OriginalMember(owner = "client!uka", name = "b", descriptor = "()I")
	@Override
	public int method9139() {
		return 0;
	}

	@OriginalMember(owner = "client!uka", name = "a", descriptor = "()Lclient!dea;")
	@Override
	public PcmStream method9142() {
		@Pc(11) Node_Sub16 local11;
		do {
			local11 = (Node_Sub16) this.aClass341_72.next();
			if (local11 == null) {
				return null;
			}
		} while (local11.aClass2_Sub6_Sub2_1 == null);
		return local11.aClass2_Sub6_Sub2_1;
	}

	@OriginalMember(owner = "client!uka", name = "c", descriptor = "()Lclient!dea;")
	@Override
	public PcmStream firstSubStream() {
		@Pc(9) Node_Sub16 local9 = (Node_Sub16) this.aClass341_72.head();
		if (local9 == null) {
			return null;
		} else if (local9.aClass2_Sub6_Sub2_1 == null) {
			return this.method9142();
		} else {
			return local9.aClass2_Sub6_Sub2_1;
		}
	}

	@OriginalMember(owner = "client!uka", name = "a", descriptor = "([ILclient!dha;IIII)V")
	private void method8542(@OriginalArg(0) int[] arg0, @OriginalArg(1) Node_Sub16 arg1, @OriginalArg(2) int arg2, @OriginalArg(3) int arg3, @OriginalArg(4) int arg4) {
		if ((this.aClass2_Sub6_Sub1_4.anIntArray56[arg1.anInt2196] & 0x4) != 0 && arg1.anInt2211 < 0) {
			@Pc(34) int local34 = this.aClass2_Sub6_Sub1_4.anIntArray57[arg1.anInt2196] / AudioChannel.globalSampleRate;
			while (true) {
				@Pc(44) int local44 = (local34 + 1048575 - arg1.anInt2192) / local34;
				if (local44 > arg3) {
					arg1.anInt2192 += arg3 * local34;
					break;
				}
				arg1.aClass2_Sub6_Sub2_1.read(arg0, arg4, local44);
				arg1.anInt2192 += local34 * local44 - 1048576;
				arg3 -= local44;
				arg4 += local44;
				@Pc(75) int local75 = AudioChannel.globalSampleRate / 100;
				@Pc(79) int local79 = 262144 / local34;
				if (local79 < local75) {
					local75 = local79;
				}
				@Pc(91) SoundPcmStream local91 = arg1.aClass2_Sub6_Sub2_1;
				if (this.aClass2_Sub6_Sub1_4.anIntArray49[arg1.anInt2196] == 0) {
					arg1.aClass2_Sub6_Sub2_1 = Static730.method3346(arg1.aClass2_Sub49_Sub1_1, local91.method3340(), local91.method3341(), local91.method3348());
				} else {
					arg1.aClass2_Sub6_Sub2_1 = Static730.method3346(arg1.aClass2_Sub49_Sub1_1, local91.method3340(), 0, local91.method3348());
					this.aClass2_Sub6_Sub1_4.method943(arg1.aClass2_Sub11_1.aShortArray16[arg1.anInt2205] < 0, arg1);
					arg1.aClass2_Sub6_Sub2_1.method3315(local75, local91.method3341());
				}
				if (arg1.aClass2_Sub11_1.aShortArray16[arg1.anInt2205] < 0) {
					arg1.aClass2_Sub6_Sub2_1.method3318(-1);
				}
				local91.method3320(local75);
				local91.read(arg0, arg4, arg2 - arg4);
				if (local91.method3336()) {
					this.aClass2_Sub6_Sub3_2.method5889(local91);
				}
			}
		}
		arg1.aClass2_Sub6_Sub2_1.read(arg0, arg4, arg3);
	}

	@OriginalMember(owner = "client!uka", name = "b", descriptor = "([III)V")
	@Override
	public void read(@OriginalArg(0) int[] arg0, @OriginalArg(1) int arg1, @OriginalArg(2) int arg2) {
		this.aClass2_Sub6_Sub3_2.read(arg0, arg1, arg2);
		for (@Pc(17) Node_Sub16 local17 = (Node_Sub16) this.aClass341_72.head(); local17 != null; local17 = (Node_Sub16) this.aClass341_72.next()) {
			if (!this.aClass2_Sub6_Sub1_4.method921(local17)) {
				@Pc(29) int local29 = arg1;
				@Pc(31) int local31 = arg2;
				do {
					if (local17.anInt2193 >= local31) {
						this.method8542(arg0, local17, local29 + local31, local31, local29);
						local17.anInt2193 -= local31;
						break;
					}
					this.method8542(arg0, local17, local31 + local29, local17.anInt2193, local29);
					local29 += local17.anInt2193;
					local31 -= local17.anInt2193;
				} while (!this.aClass2_Sub6_Sub1_4.method945(local17, arg0, local29, local31));
			}
		}
	}
}
