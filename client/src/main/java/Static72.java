import org.openrs2.deob.annotation.OriginalArg;
import org.openrs2.deob.annotation.OriginalMember;
import org.openrs2.deob.annotation.Pc;

public final class Static72 {

	@OriginalMember(owner = "client!cf", name = "c", descriptor = "J")
	public static long aLong52;

	@OriginalMember(owner = "client!cf", name = "a", descriptor = "(I[I)Ljava/lang/String;")
	public static String method1527(@OriginalArg(1) int[] arg0) {
		@Pc(7) StringBuffer local7 = new StringBuffer();
		@Pc(9) int local9 = Static331.anInt5459;
		for (@Pc(11) int local11 = 0; local11 < arg0.length; local11++) {
			@Pc(19) QuestType local19 = client.questTypeList.method1094(arg0[local11]);
			if (local19.anInt5724 != -1) {
				@Pc(34) Class23 local34 = (Class23) Static452.aClass82_149.get(local19.anInt5724);
				if (local34 == null) {
					@Pc(42) Class409 local42 = Static735.method9389(client.js5Archive8, local19.anInt5724, 0);
					if (local42 != null) {
						local34 = Static163.aClass19_17.method7955(local42, true);
						Static452.aClass82_149.put(local34, local19.anInt5724);
					}
				}
				if (local34 != null) {
					Static186.aClass23Array5[local9] = local34;
					local7.append(" <img=").append(local9).append(">");
					local9++;
				}
			}
		}
		return local7.toString();
	}
}
