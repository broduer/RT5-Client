...
public final class TextureOp37 extends TextureOp {
	...
	public void decode(@OriginalArg(1) Buffer buf, @OriginalArg(2) int code) {
		if (code == 0) {
			this.anInt6598 = buf.g2();
		} else if (code == 1) {
			this.anInt6600 = buf.g2();
		} else if (code == 2) {
			this.anInt6594 = buf.g2();
		} else if (code == 3) {
			this.anInt6597 = buf.g2();
		} else if (code == 4) {
			this.anInt6593 = buf.g2();
		} else if (code == 5) {
			this.anInt6590 = buf.g2();
		} else if (code == 6) {
			this.anInt6586 = buf.g2();
		}
	}
	...
	private boolean method5858(@OriginalArg(0) int arg0, @OriginalArg(1) int arg1) {
		...
		@Pc(33) int local33 = TextureOp.COSINE[local13 * 255 >> 12 & 0xFF];
	...
	public int[] getMonochromeOutput(@OriginalArg(1) int y) {
		...
		if (super.monochromeImageCache.invalid) {
			@Pc(27) int local27 = Static273.normalizedY[y] - 2048;
			for (@Pc(29) int local29 = 0; local29 < Static608.width; local29++) {
				@Pc(37) int local37 = Static54.normalizedX[local29] - 2048;
	...
	private boolean method5859(@OriginalArg(1) int arg0, @OriginalArg(2) int arg1) {
		...
		@Pc(28) int local28 = TextureOp.COSINE[local18 * 255 >> 12 & 0xFF];
