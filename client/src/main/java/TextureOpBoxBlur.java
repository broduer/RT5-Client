...
public final class TextureOpBoxBlur extends TextureOp {
	...
	public int[][] getColorOutput(@OriginalArg(0) int y) {
		...
		if (super.colorImageCache.invalid) {
			...
			@Pc(75) int local75;
			@Pc(77) int local77;
			for (@Pc(55) int local55 = y - this.anInt4871; local55 <= y + this.anInt4871; local55++) {
				@Pc(67) int[][] local67 = this.method9420(0, Static489.anInt7364 & local55);
				@Pc(71) int[][] local71 = new int[3][Static608.width];
				@Pc(73) int local73 = 0;
				local75 = 0;
				local77 = 0;
				...
				while (Static608.width > local144) {
	...
	public int[] getMonochromeOutput(@OriginalArg(1) int y) {
		...
		if (super.monochromeImageCache.invalid) {
			...
			for (@Pc(48) int local48 = y - this.anInt4871; local48 <= y + this.anInt4871; local48++) {
				...
				local42[local48 + this.anInt4871 - y] = local61;
			}
			for (@Pc(146) int local146 = 0; local146 < Static608.width; local146++) {
	...
	public void decode(@OriginalArg(1) Buffer buf, @OriginalArg(2) int code) {
		if (code == 0) {
			this.anInt4866 = buf.g1();
		} else if (code == 1) {
			this.anInt4871 = buf.g1();
		} else if (code == 2) {
