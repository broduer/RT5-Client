...
public final class TextureOpBricks extends TextureOp {
	...
	private void method4069() {
		@Pc(12) Random random = new Random(this.anInt4541);
		...
		for (@Pc(77) int local77 = 0; local77 < this.anInt4541; local77++) {
			...
			if (local77 > 0) {
				...
				local98 = (Static623.nextInt(4096, random) - 2048) * this.anInt4539 >> 12;
			...
			for (local86 = 0; local86 < this.anInt4554; local86++) {
				if (local86 > 0) {
					...
					@Pc(152) int local152 = (Static623.nextInt(4096, random) - 2048) * this.anInt4553 >> 12;
					local98 += local35 * local152 >> 12;
					this.anIntArrayArray109[local77][local86] = local98 + this.anIntArrayArray109[local77][local86 - 1];
				}
				this.anIntArrayArray110[local77][local86] = this.anInt4546 > 0 ? 4096 - Static623.nextInt(this.anInt4546, random) : 4096;
			}
	...
	public void decode(@OriginalArg(1) Buffer buf, @OriginalArg(2) int code) {
		if (code == 0) {
			this.anInt4554 = buf.g1();
		} else if (code == 1) {
			this.anInt4541 = buf.g1();
		} else if (code == 2) {
			this.anInt4553 = buf.g2();
		} else if (code == 3) {
			this.anInt4539 = buf.g2();
		} else if (code == 4) {
			this.anInt4549 = buf.g2();
		} else if (code == 5) {
			this.anInt4545 = buf.g2();
		} else if (code == 6) {
			this.anInt4542 = buf.g2();
		} else if (code == 7) {
			this.anInt4546 = buf.g2();
		}
	}
	...
	public int[] getMonochromeOutput(@OriginalArg(1) int y) {
		...
		if (super.monochromeImageCache.invalid) {
			@Pc(24) int local24 = 0;
			@Pc(31) int local31;
			for (local31 = this.anInt4545 + Static273.normalizedY[y]; local31 < 0; local31 += 4096) {
			}
			while (local31 > 4096) {
				local31 -= 4096;
			}
			...
			if (this.anInt4547 + local102 < local31 && local31 < local95 - this.anInt4547) {
				for (@Pc(123) int local123 = 0; local123 < Static608.width; local123++) {
					@Pc(127) int local127 = 0;
					@Pc(137) int local137 = local90 ? this.anInt4549 : -this.anInt4549;
					@Pc(148) int local148;
					for (local148 = Static54.normalizedX[local123] + (local137 * this.anInt4543 >> 12); local148 < 0; local148 += 4096) {
					}
					while (local148 > 4096) {
						local148 -= 4096;
					}
				...
				Static734.fill(local18, 0, Static608.width, 0);
			}
		}
		return local18;
	}
