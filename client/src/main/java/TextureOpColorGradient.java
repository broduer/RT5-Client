...
public final class TextureOpColorGradient extends TextureOp {
	...
	public int[][] getColorOutput(@OriginalArg(0) int y) {
		...
		if (super.colorImageCache.invalid) {
			@Pc(29) int[] local29 = this.method9429(y, 0);
			@Pc(33) int[] local33 = local19[0];
			@Pc(37) int[] local37 = local19[1];
			@Pc(41) int[] local41 = local19[2];
			for (@Pc(43) int local43 = 0; local43 < Static608.width; local43++) {
	...
	public void decode(@OriginalArg(1) Buffer buf, @OriginalArg(2) int code) {
		if (code == 0) {
			...
			if (local14 == 0) {
				this.anIntArrayArray125 = new int[buf.g1()][4];
				for (@Pc(28) int local28 = 0; local28 < this.anIntArrayArray125.length; local28++) {
					this.anIntArrayArray125[local28][0] = buf.g2();
					this.anIntArrayArray125[local28][1] = buf.g1() << 4;
					this.anIntArrayArray125[local28][2] = buf.g1() << 4;
					this.anIntArrayArray125[local28][3] = buf.g1() << 4;
				}
			} else {
