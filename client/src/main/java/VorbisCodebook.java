...
public final class VorbisCodebook {
	...
	public VorbisCodebook() {
		...
		this.anInt6824 = VorbisSound.readBits(16);
		this.anInt6823 = VorbisSound.readBits(24);
		...
		if (local23) {
			...
			while (local27 < this.anInt6823) {
				@Pc(41) int local41 = VorbisSound.readBits(Static434.bitCount(this.anInt6823 - local27));
			...
			for (local32 = 0; local32 < this.anInt6823; local32++) {
				if (local66 && VorbisSound.readBit() == 0) {
					...
					this.anIntArray543[local32] = VorbisSound.readBits(5) + 1;
				}
			}
		}
		this.method6117();
		...
		if (local27 > 0) {
			...
			local43 = VorbisSound.readBits(4) + 1;
			...
			for (@Pc(140) int local140 = 0; local140 < local127; local140++) {
				this.anIntArray544[local140] = VorbisSound.readBits(local43);
			}
	...
	public int method6118() {
		@Pc(1) int local1;
		for (local1 = 0; this.anIntArray545[local1] >= 0; local1 = VorbisSound.readBit() == 0 ? local1 + 1 : this.anIntArray545[local1]) {
		}
