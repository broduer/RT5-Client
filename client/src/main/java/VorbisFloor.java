...
public final class VorbisFloor {
	...
	public VorbisFloor() {
		@Pc(4) int local4 = VorbisSound.readBits(16);
		if (local4 != 1) {
			throw new RuntimeException();
		}
		@Pc(15) int local15 = VorbisSound.readBits(5);
		@Pc(17) int local17 = 0;
		...
		for (local27 = 0; local27 < local17; local27++) {
			this.anIntArray84[local27] = VorbisSound.readBits(3) + 1;
			local78 = this.anIntArray79[local27] = VorbisSound.readBits(2);
			if (local78 != 0) {
				this.anIntArray83[local27] = VorbisSound.readBits(8);
			}
			local78 = 0x1 << local78;
			@Pc(95) int[] local95 = new int[local78];
			this.anIntArrayArray22[local27] = local95;
			for (local102 = 0; local102 < local78; local102++) {
				local95[local102] = VorbisSound.readBits(8) - 1;
			}
		}
		this.anInt1008 = VorbisSound.readBits(2) + 1;
		local78 = VorbisSound.readBits(4);
		...
		for (@Pc(168) int local168 = 0; local168 < local15; local168++) {
			...
			for (@Pc(176) int local176 = 0; local176 < this.anIntArray84[local174]; local176++) {
				this.anIntArray81[local131++] = VorbisSound.readBits(local78);
			}
		}
	...
	public boolean method1089() {
		@Pc(6) boolean local6 = VorbisSound.readBit() != 0;
		if (!local6) {
			return false;
		}
		...
		@Pc(42) int local42 = Static434.bitCount(local36 - 1);
		Static47.anIntArray82[0] = VorbisSound.readBits(local42);
		Static47.anIntArray82[1] = VorbisSound.readBits(local42);
		@Pc(54) int local54 = 2;
		for (@Pc(56) int local56 = 0; local56 < this.anIntArray78.length; local56++) {
			...
			@Pc(80) int local80 = 0;
			if (local72 > 0) {
				local80 = VorbisSound.codebooks[this.anIntArray83[local62]].method6118();
			}
			for (@Pc(92) int local92 = 0; local92 < local67; local92++) {
				@Pc(102) int local102 = this.anIntArrayArray22[local62][local80 & local78];
				local80 >>>= local72;
				Static47.anIntArray82[local54++] = local102 >= 0 ? VorbisSound.codebooks[local102].method6118() : 0;
			}
		}
		return true;
	}
