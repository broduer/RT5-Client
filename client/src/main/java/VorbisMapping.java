...
public final class VorbisMapping {
	...
	public VorbisMapping() {
		...
		this.anInt9503 = VorbisSound.readBit() == 0 ? 1 : VorbisSound.readBits(4) + 1;
		if (VorbisSound.readBit() != 0) {
			VorbisSound.readBits(8);
		}
		VorbisSound.readBits(2);
		if (this.anInt9503 > 1) {
			this.anInt9504 = VorbisSound.readBits(4);
		}
		...
		for (@Pc(42) int local42 = 0; local42 < this.anInt9503; local42++) {
			VorbisSound.readBits(8);
			this.anIntArray738[local42] = VorbisSound.readBits(8);
			this.anIntArray739[local42] = VorbisSound.readBits(8);
		}
	}
}
